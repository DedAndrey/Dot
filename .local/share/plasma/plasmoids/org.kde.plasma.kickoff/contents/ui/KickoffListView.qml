...
FocusScope {
    ...
    PC3.ScrollView {
        ...
        PC3.ScrollBar.vertical.visible: scrollBarVisibility
        focus: true

        ListView {
            currentIndex: 0
            onCurrentIndexChanged: if(plasmoid.configuration.switchCategoriesOnHover)select();
            id: listView
            ...
            MouseArea {
                ...
                onClicked:if(!plasmoid.configuration.switchCategoriesOnHover)listView.select();
                ...
                onReleased: {
                    var mapped = listView.mapToItem(listView.contentItem, mouse.x, mouse.y);
                    var item = listView.itemAt(mapped.x, mapped.y);
                    if (mouse.button === Qt.LeftButton&plasmoid.configuration.scrollableApps &!item){ currentItem.activate();}
                ...
                onPositionChanged: {if(!scrollingMode){
                    var mapped = listView.mapToItem(listView.contentItem, mouse.x, mouse.y);
                    var item = listView.itemAt(mapped.x, mapped.y);
                ...
                onPressAndHold: {
                    ...
                    if (mouse.button === Qt.RightButton&!item)rightTap=true
                        if (mouse.source == Qt.MouseEventSynthesizedByQt) {
                            tapAndHold = true;
                            positionChanged(mouse);
                        }
                ...
                onWheel: {
                    if (rightTap){
                        if (wheel.angleDelta.y > 0){
                            plasmoid.configuration.listIconSize=plasmoid.configuration.listIconSize+16
                        }else{
                            plasmoid.configuration.listIconSize=plasmoid.configuration.listIconSize-16}
                    }else if(!scrollBarVisibility||plasmoid.configuration.scrollableApps){
                        scrollingMode=true
                        wheel.angleDelta.y > 0?keyNavUp():keyNavDown()
